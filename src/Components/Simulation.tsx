...
const Simulation: React.FC = () => {
  ...
  const proceedSimulation = () => {
    ...
    participants.forEach((i) => i.flipCoin());
  ...
  return (
    <div>
      <div className={classes.actions}>
        <Rules />
        <SetParticipants
          disability={disableInput}
          spitParticipants={(p) => {
            setParticipantNumber(p);
          }}
        />
        <Button onClick={proceedSimulation} type="primary">
          {iterations === 0 ? "START" : "NEXT"} <RightCircleOutlined />
        </Button>
        {!(iterations === 0) && (
          <div
            style={{
              display: "flex",
              marginRight: 10,
              marginTop: window.innerWidth < 700 ? 10 : 0,
              marginLeft: 10,
              padding: 8,
      ...
      >
        {participants.map((i) => {
          return (
            <ParticipantInField key={i.participantID} outcome={i.outcome} />
          );
        })}
      </div>
